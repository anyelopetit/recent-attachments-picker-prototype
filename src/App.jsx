...
function SentAttachmentCard({ attachment }) {
  return (
    <div className="sent-attachment-card">
      <UploadBadge upload={attachment} />
      <span>
        <strong>{attachment.filename}</strong>
        <small>{attachment.fileType} · {formatBytes(attachment.sizeBytes)}</small>
      </span>
    </div>
  )
}
...
function HellotextLogo() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="150" height="43" viewBox="0 0 224 43" className="hellotext-logo" role="img" aria-labelledby="hellotext-logo-title">
      <title id="hellotext-logo-title">hellotext</title>
...
export default function App() {
  ...
  const [message, setMessage] = useState('Hi! Sharing the latest sizing chart and pricing sheet here.')
  const [attachments, setAttachments] = useState(() => getDraftAttachments())
  ...
  useEffect(() => {
    saveDraftAttachments(attachments)
  }, [attachments])
  ...
  function addAttachment(upload) {
    if (isDuplicateAttachment(attachments, upload)) {
      setNotice(`${upload.filename} is already attached to this draft.`)
      return false
    }

    setAttachments((current) => [...current, upload])
    setNotice(`${upload.filename} added to the message.`)
    return true
  }
  ...
  function handleLocalFiles(event) {
    ...
    files.forEach((file) => {
      ...
      const upload = makeLocalUpload(file)
  ...
  function toggleRecentSelection(upload) {
    if (isDuplicateAttachment(attachments, upload)) {
      setNotice(`${upload.filename} is already attached. Remove it from the draft first to change it.`)
      return
    }

    setSelectedIds((current) => (
      current.includes(upload.id)
        ? current.filter((id) => id !== upload.id)
        : [...current, upload.id]
    ))
  }

  function addSelectedRecent() {
    const selectedUploads = recentUploads.filter((upload) => selectedIds.includes(upload.id))
    let addedCount = 0

    selectedUploads.forEach((upload) => {
      if (addAttachment(upload)) addedCount += 1
    })

    setSelectedIds([])
    setPickerOpen(false)
    if (addedCount > 0) setNotice(`${addedCount} recent attachment${addedCount === 1 ? '' : 's'} added.`)
  }

  function removeAttachment(id) {
    const removed = attachments.find((attachment) => attachment.id === id)
    setAttachments((current) => current.filter((attachment) => attachment.id !== id))
    if (removed) setNotice(`${removed.filename} removed from this draft.`)
  }
  ...
  function sendMessage() {
    ...
    if (!body && sentAttachments.length === 0) return
    ...
    markUploadsUsed(sentAttachments.map((attachment) => attachment.id))
    setMessage('')
    setAttachments([])
    setSelectedIds([])
    setRecentUploads(getRecentUploads())
    setNotice('Message sent in demo mode. No data left this browser.')
  }
  ...
  function resetDemo() {
    ...
    setRecentUploads(getRecentUploads())
    setNotice('Demo storage reset to seeded UserUpload records.')
  }

  const visibleUploads = emptyMode
    ? []
    : recentUploads.filter((upload) => upload.filename.toLowerCase().includes(query.toLowerCase()))
    ...
    <main className="app-shell">
      ...
      <section className="demo-grid" aria-label="Interactive chat demo">
        <section className="phone-frame" aria-label="Hellotext chat composer">
          <div className="chat-header">
            ...
            <span className="channel-pill">SMS + MMS</span>
          </div>
              ...
              <div className={`bubble ${sentMessage.direction}`} key={sentMessage.id}>
                ...
                {sentMessage.attachments.length > 0 && (
                  <div className="sent-attachments" aria-label={`Attachments sent with ${sentMessage.id}`}>
                    {sentMessage.attachments.map((attachment) => (
                      <SentAttachmentCard attachment={attachment} key={attachment.id} />
                    ))}
                  </div>
                )}
          ...
          <div className="composer-card">
            ...
            <div className="composer-actions">
              ...
              <button className="send-button" type="button" onClick={sendMessage} disabled={!message.trim() && attachments.length === 0}>
                <Icon name="send" />
                Send demo
              </button>
            </div>
      ...
      {pickerOpen && (
        <div className="modal-backdrop" role="presentation">
          <section className="picker-panel" role="dialog" aria-modal="true" aria-labelledby="recent-title">
            <div className="picker-header">
              <div>
                <p className="eyebrow">Recent attachments</p>
                <h2 id="recent-title">Pick files already uploaded</h2>
              </div>
              <button className="icon-button" type="button" aria-label="Close recent attachments" onClick={() => setPickerOpen(false)}>
                <Icon name="x" />
              </button>
            </div>

            <div className="search-row">
              <label htmlFor="attachment-search">Search by filename</label>
              <input
                id="attachment-search"
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder="Try pricing, sizing, policy..."
              />
            </div>

            <div className="state-controls" aria-label="Demo state controls">
              <button type="button" onClick={() => setEmptyMode((value) => !value)}>{emptyMode ? 'Show files' : 'Simulate empty'}</button>
              <button type="button" onClick={() => setPickerError('Could not load recent attachments. Check your connection and retry.')}>Simulate error</button>
              {pickerError && <button type="button" onClick={() => setPickerError('')}>Retry</button>}
            </div>

            {loadingRecent ? (
              <div className="loading-grid" aria-label="Loading recent attachments">
                <div /><div /><div />
              </div>
            ) : pickerError ? (
              <div className="empty-state error-state" role="alert">
                <strong>Recent files are unavailable</strong>
                <p>{pickerError}</p>
              </div>
            ) : visibleUploads.length === 0 ? (
              <div className="empty-state">
                <strong>No reusable files found</strong>
                <p>Upload a file once, then it will appear here for fast reuse in future messages.</p>
              </div>
            ) : (
              <div className="upload-grid" aria-label="Recent upload results">
                {visibleUploads.map((upload) => {
                  const attached = isDuplicateAttachment(attachments, upload)
                  const selected = selectedIds.includes(upload.id)

                  return (
                    <button
                      className={`upload-card ${selected ? 'selected' : ''} ${attached ? 'attached' : ''}`}
                      key={upload.id}
                      type="button"
                      aria-pressed={selected}
                      onClick={() => toggleRecentSelection(upload)}
                    >
            ...
            <div className="picker-footer">
              <span>{selectedIds.length} selected for this draft</span>
              <div>
                <button className="ghost-button" type="button" onClick={() => setPickerOpen(false)}>Cancel</button>
                <button className="send-button" type="button" onClick={addSelectedRecent} disabled={selectedIds.length === 0}>Add selected</button>
              </div>
            </div>
          </section>
        </div>
